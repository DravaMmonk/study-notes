...
- Use **average return** to **estimate** value function
	- *instead of solving $v(n)$ in Model-based RL*
	$$
...
- Use *Generalised Policy Iteration* (GPI) Framework to improve efficiency
	- In an iteration, instead of do "Policy Evaluation → Policy Improvement" **once**, we repeat by doing sub-process of it
- For each episode $e$:
...
---
## $\alpha$-style MC
- In practice, we can also use learning rate $\alpha$ for replacing strict mean:
	$$
